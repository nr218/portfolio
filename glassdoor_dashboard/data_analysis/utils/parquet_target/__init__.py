"""Please note that this code is a copy of my code submitted in csci_utils"""
...
@delayed(pure=True)
def touch(path, storage_options=None, _dep=None):  # pragma: no cover
    fs, token, paths = get_fs_token_paths(path, storage_options=storage_options)
    with fs.open(path, mode="wb"):
        pass


class BaseDaskTarget(Target):  # pragma: no cover
...
class TargetOutput:  # pragma: no cover
    def __init__(
        self,
    ...
    def __call__(self, task):
        ...
        filename = self.file_pattern.format(ext=self.ext, task=task)
...
class ParquetTarget(BaseDaskTarget):
    ...
    @classmethod
    def _write(cls, collection, path, **kwargs):
        return to_parquet(collection, path=path, **kwargs)
